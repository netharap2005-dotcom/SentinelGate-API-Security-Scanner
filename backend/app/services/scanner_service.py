    ...
    for item in live_endpoints[:3]:
        ...
        if len(missing) >= 2:
            findings.append(build_finding(
                ...
                description="The API response is missing important security headers that help protect against browser-based attacks and insecure client-side behaviour.",
    ...
    for item in live_endpoints:
        ...
        if exposed:
            findings.append(build_finding(
                ...
                description="The API response appears to expose sensitive fields that should not be returned to normal users.",
    ...
    for path in sql_paths:
        ...
        if response.status_code >= 500 or any(word in body for word in suspicious_indicators):
            findings.append(build_finding(
                ...
                description="The endpoint may be vulnerable to SQL injection because SQL-like input caused abnormal database or server behaviour.",
    ...
    for path in auth_paths:
        ...
        if response.status_code == 200 and any(word in body for word in success_words):
            findings.append(build_finding(
                ...
                description="The login endpoint may have weak authentication handling because invalid credentials produced a success-like response.",
...
def test_rate_limit(base_url, live_endpoints):
    ...
    if success_count >= 10 and rate_limited_count == 0:
        findings.append(build_finding(
            ...
            description="The endpoint may not enforce request throttling, allowing repeated requests without rate limiting.",
    ...
    for path in user_paths[:2]:
        ...
        if response.status_code == 200 and any(word in body for word in ["email", "username", "address", "phone"]):
            findings.append(build_finding(
                ...
                description="The endpoint may expose object-level data directly through an object ID without proper authorization checks.",
    ...
    for item in live_endpoints[:3]:
        ...
        if allow_origin == "*" or (
            ...
            findings.append(build_finding(
                ...
                description="The API may allow unsafe cross-origin access, which could expose data to untrusted websites.",
    ...
    for path in debug_paths:
        ...
        if response.status_code == 200 and any(word in body for word in indicators):
            findings.append(build_finding(
                ...
                description="Debug or configuration endpoints may be exposed, which can reveal sensitive application details.",
    ...
    for path in ssrf_paths:
        ...
        if response.status_code == 200 and any(word in body for word in ["ami-id", "instance-id", "metadata"]):
            findings.append(build_finding(
                ...
                description="The endpoint may allow server-side requests to internal or restricted URLs.",
